package me.taste2plate.app.data.auth
...
class AuthIntercepter(private val consumerKey: String, private val consumerSecret: String) : Interceptor {
    ...
    var TAG = "YOUR_APP_TAG"
    var TIME_SERVER = "0.europe.pool.ntp.org"
   ...
   /* fun getOauth_timestamp():String {
        val stamp = (System.currentTimeMillis() / 1000.0).toLong()
        val localTime= StringBuilder(stamp.toString()).toString()
        var setverTime=0L
        try {
            val timeClient = NTPUDPClient()
            val inetAddress = InetAddress.getByName(TIME_SERVER)
            val timeInfo = timeClient.getTime(inetAddress)
            setverTime = timeInfo.message.transmitTimeStamp.time/1000
            return setverTime.toString()
        } catch (e: Exception) {
            Log.v(TAG, "Time server error - " + e.localizedMessage)
            return localTime
        }
        return localTime
    }*/
